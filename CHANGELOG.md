...
- **Repository Synchronization**: Executed full Executive Protocol — fetched all remote branches, fast-forwarded all 5 local branches (main + 4 feature branches) to identical commit state.
- **Documentation Update**: Updated CHANGELOG with missing v1.1.65–v1.1.68 entries.

## [1.1.68] - 2026-06-23

### Added

- **AI-Generated Pitch Subject Lines**: Pitches now include dynamic, AI-generated subject lines instead of the hardcoded "Proposal for Psytrance Night Residency" string.
- **Schema Update**: Added `generated_subject` column to `outreach_leads` table.

### Removed

- **Deduplicated Test Files**: Removed `test_google_maps_fallback.py` and `test_token_budget.py` — test logic consolidated into existing test suites.

## [1.1.67] - 2026-06-22

### Added

- **Google Maps Places API Fallback**: When Playwright CSS selectors fail, the scraper now falls back to the Google Maps Places API for reliable venue discovery.
- **API Key Env Var**: Added `GOOGLE_MAPS_API_KEY` to `.env.template`.

## [1.1.66] - 2026-06-21

### Added

- **Social Media Automation (Phase 49)**: Implemented Instagram and Facebook DM generation and ingestion pipeline.
- **Schema Update**: Added DM-specific tables and fields for social media outreach tracking.
- **Dashboard DM View**: Added DM management interface to the venue detail page.

## [1.1.65] - 2026-06-20

### Added

- **Token Budget Alerts**: Implemented per-run and per-day OpenAI token budget limits. Warnings are logged and displayed on the System dashboard when budgets are exceeded.
- **Config Extension**: Added `max_tokens_per_day` and `max_tokens_per_run` to `database/config.json`.
- **Token Budget Test Suite**: Created `tests/test_token_budget.py`.
